	...
	factory: (require) => {
		...
		*/
		function Row(props) {
			...
			const displayName = row.kind === "prompt" ? t(`prompt.${row.name}.name`) : row.kind === "approval" ? t("approval.name") : row.kind === "guard" ? t(`guard.${row.name}.name`) : row.name;
			const displayDesc = row.kind === "prompt" ? t(`prompt.${row.name}.desc`) : row.kind === "approval" ? t("approval.desc") : row.kind === "guard" ? t(`guard.${row.name}.desc`) : row.description;
