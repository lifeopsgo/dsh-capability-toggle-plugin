...
const PS_READONLY_WRITE = /\b(?:Set-Content|Add-Content|Clear-Content|Out-File|New-Item|Export-Csv)\b|(?:^|[;|&({=\r\n])\s*(?:ni|clc|epcsv)\b/i;
...
*/
const DANGEROUS_SHELL = /(?<![-\w])\brm\b[^\n;|&]*\s+(?:--(?:recursive|force)\b|-\w*[rRfF]\w*\b)|\bdd\b[^\n;|&]*\b(?:if|of)=|\bmkfs\b|\bchmod\s+(-R\s+)?777\b|\|\s*(sudo\s+)?(sh|bash)\b|:\(\)\s*\{\s*:\s*\|\s*:|>\s*\/dev\/(sd|nvme|disk)/;
const PS_DANGEROUS = /\b(?:Format-Volume|Clear-Disk|Stop-Computer|Restart-Computer)\b|\bRemove-Item\b(?:[^\n;|]|`\r?\n)*-(?:R(?:e(?:c(?:u(?:r(?:s(?:e)?)?)?)?)?)?|Fo(?:r(?:c(?:e)?)?)?)\b|(?:^|[;|&({=\r\n])\s*(?:rm|del|ri|rmdir|erase|rd)\b(?:[^\n;|]|`\r?\n)*-(?:R(?:e(?:c(?:u(?:r(?:s(?:e)?)?)?)?)?)?|Fo(?:r(?:c(?:e)?)?)?)\b/i;
const SHELL_ARGUMENT = String.raw`(?:"[^"\r\n]*"|'[^'\r\n]*'|[^\s;|&"']+)`;
const PS_ENCODED = new RegExp(String.raw`\b(?:pwsh|powershell)(?:\.exe)?(?:[ \t]+-(?:(?:ExecutionPolicy|ep|WindowStyle|InputFormat|OutputFormat)[ \t]+${SHELL_ARGUMENT}|[a-z]\w*))*[ \t]+-e(?:n(?:c(?:o(?:d(?:e(?:d(?:command)?)?)?)?)?)?)?\b`, "i");
/**
...
function buildConfirmDetail(toolName, args) {
	if ((toolName === "bash" || toolName === "pwsh") && typeof args["command"] === "string") return bound(args["command"]);
	let json;
	try {
	...
	const disposeConfirmStream = ctx.webServer.register({
		...
		handler: (req, res) => guard(ctx, res, async () => {
			await handleMethod(req, res, "GET", async () => {
				await handleConfirmStream(req, res, center, auth);
			});
		})
	});
...
function resolveAuth(ctx) {
	const fn = ctx.get("connection")?.requestRejection;
...
*/
function handleConfirmStream(req, res, center, auth) {
	const rejection = auth(req);
	if (rejection !== void 0) {
		sendJson(res, rejection, { error: "unauthorized" });
		return;
	...
	res.flushHeaders();
	const off = center.subscribe(session, {
